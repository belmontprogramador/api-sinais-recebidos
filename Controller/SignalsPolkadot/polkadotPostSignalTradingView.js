// polkadotPostSignalTradingView.js
...
// Função para enviar o sinal para a rota '/comparesignalresultoperationpolkadot'
const sendToCompareSignal = async (signal) => {
  try {
    const response = await axios.post('http://localhost:3004/comparesignalresultoperationpolkadot', {
      ticker: signal.ticker,
      orderType: signal.orderType,
      price: signal.price,
      interval: signal.interval,
    });
    console.log('Sinal enviado para /comparesignalresultoperationpolkadot com sucesso:', response.data);
  } catch (error) {
    console.error('Erro ao enviar o sinal para /comparesignalresultoperationpolkadot:', error.message);
    console.error('Dados do erro:', {
      url: 'http://localhost:3004/comparesignalresultoperationpolkadot',
      data: {
        ticker: signal.ticker,
        orderType: signal.orderType,
        price: signal.price,
...
const insertSignalIntoCorrectTable = async (ticker, orderType, price, interval, timenow) => {
  ...
  switch (parseInt(interval)) {
    case 1:
      signal = await prisma.polkadotSignalOneMinute.create({
        data: { ticker, orderType, price: parseFloat(price), interval, createdAt: new Date(timenow) },
      });
      break;
    case 5:
      signal = await prisma.polkadotSignalFiveMinute.create({
        data: { ticker, orderType, price: parseFloat(price), interval, createdAt: new Date(timenow) },
      });
      break;
    case 15:
      signal = await prisma.polkadotSignalFifteenMinute.create({
        data: { ticker, orderType, price: parseFloat(price), interval, createdAt: new Date(timenow) },
      });
      break;
    case 45:
      signal = await prisma.polkadotSignalFortyFiveMinute.create({
        data: { ticker, orderType, price: parseFloat(price), interval, createdAt: new Date(timenow) },
      });
      break;
    case 60:
      signal = await prisma.polkadotSignalOneHour.create({
        data: { ticker, orderType, price: parseFloat(price), interval, createdAt: new Date(timenow) },
      });
      break;
    case 240:
      signal = await prisma.polkadotSignalFourHours.create({
        data: { ticker, orderType, price: parseFloat(price), interval, createdAt: new Date(timenow) },
      });
      break;
    case 1440:
      signal = await prisma.polkadotSignalOneDay.create({
        data: { ticker, orderType, price: parseFloat(price), interval, createdAt: new Date(timenow) },
      });
      break;
    case 10080:
      signal = await prisma.polkadotSignalOneWeek.create({
        data: { ticker, orderType, price: parseFloat(price), interval, createdAt: new Date(timenow) },
      });
      break;
    case 43200:
      signal = await prisma.polkadotSignalOneMonth.create({
        data: { ticker, orderType, price: parseFloat(price), interval, createdAt: new Date(timenow) },
      });
      break;
    case 129600:
      signal = await prisma.polkadotSignalThreeMonth.create({
        data: { ticker, orderType, price: parseFloat(price), interval, createdAt: new Date(timenow) },
      });
      break;
    case 259200:
      signal = await prisma.polkadotSignalSixMonth.create({
        data: { ticker, orderType, price: parseFloat(price), interval, createdAt: new Date(timenow) },
      });
      break;
    case 518400:
      signal = await prisma.polkadotSignalOneYear.create({
        data: { ticker, orderType, price: parseFloat(price), interval, createdAt: new Date(timenow) },
      });
      break;
    default:
...
const polkadotSignalsTradingView = async (req, res) => {
  try {
...
module.exports = polkadotSignalsTradingView;
